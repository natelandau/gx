...
class LogPanel:
    ...
    def render(self) -> Panel | None:
        ...
        entries = self._parse_entries(result.stdout, has_body=self.show_body)
        if not entries:
            return None
    ...
    def _build_panel(self, entries: list[LogEntry]) -> Panel:
        ...
        if not self.show_body:
            ...
            for entry in entries:
                row_table = self._make_table()
                self._add_row(row_table, entry)
    ...
    def _render_refs(entry: LogEntry) -> Text:
        ...
        for branch in entry.branches:
            refs.append(f" {branch} ", style="ref.branch")
            refs.append(" ")
        for tag in entry.tags:
            refs.append(f" \U0001f3f7 {tag} ", style="ref.tag")
            refs.append(" ")
        return refs
    ...
    def _add_row(table: Table, entry: LogEntry) -> None:
        ...
        refs = LogPanel._render_refs(entry)
        subject_col = Text()
        if refs.plain.strip():
            subject_col.append_text(refs)
        subject_col.append(entry.subject)
        table.add_row(entry.sha, entry.relative_time, subject_col, entry.author)

    @staticmethod
    def _parse_refs(raw_refs: str) -> tuple[list[str], list[str]]:
        """Parse a raw ref decoration string into branches and tags lists.
    ...
    def _parse_entries(raw: str, *, has_body: bool) -> list[LogEntry]:
        ...
        for record in records:
            ...
            branches, tags = LogPanel._parse_refs(raw_refs)
